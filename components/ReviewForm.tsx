...
const ReviewForm = ({ nextStep, prevStep, formData }: any) => {
  ...
  const fetchLatestQuoteId = useCallback(async (): Promise<void> => {
    try {
      ...
      if (allQuotes && allQuotes.length > 0) {
        // Get unique quote numbers (revisions share the same numero)
        const uniqueNumeros = new Set(allQuotes.map(q => q.numero));
        setLatestQuoteId(100 + uniqueNumeros.size); // 100, 101, 102...
      } else {
        setLatestQuoteId(100);
      }
    } catch (err) {
